...
**Data availability:** Mimir has no `claude_code.*` metrics before **2026-07-18**. The collector was offline for approximately 3 weeks after the June 22 bootstrap; see `observability/local/otel-collector-config.yaml` git history for details. Dashboard views before that date will show a genuine data gap, not a query error.
