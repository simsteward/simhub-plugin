...
> Developer-curated. Complements `STATUS.md` (what was built) and `INTERFACE.md` (contract).

---

## Current state (as of 2026-03-02)

- **Plugin:** Layer 4–only incident detection from session YAML (`CurDriverIncidentCount`). Incidents carry `replayFrameNum` (from `ReplayFrameNumEnd` at add time). Seek-backward does not clear incidents or reset metrics; dedup by `(CarIdx, ReplayFrameNum)` before add. Real-time `incidentEvents` push and throttled state; OBS-ready fields (`replayFrameNum`, `sessionId`) in place.
...
- **Docs:** `INTERFACE.md` is source of truth for WebSocket contract. `STATUS.md` describes what was built and open tasks; OBS entry point described. No formal “roadmap” file until this one.
...
| Item | Notes |
|------|--------|
| **Log/event stream UI** | Per `docs/PLAN-log-event-stream-SUMMARY.md`: live event stream panel (log entries + incident push + optional physics). Implementation order and phases already outlined there. |
...
| 4 | **STATUS.md date** | Update “Current Status” date when doing releases or major checkpoints. |
...
- **Status and open tasks:** `docs/STATUS.md`
- **WebSocket contract:** `docs/INTERFACE.md`
- **Log/event stream plan:** `docs/PLAN-log-event-stream-SUMMARY.md`, `docs/PLAN-log-event-stream.md`
